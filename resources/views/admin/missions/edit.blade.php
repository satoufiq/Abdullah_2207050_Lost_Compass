...
<div class="card">
    <div class="card-header">
        <h5 class="mb-0">Edit Mission: {{ $mission->title }}</h5>
    </div>
    <div class="card-body">
        <form action="{{ route('admin.missions.update', $mission->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="title" name="title" value="{{ old('title', $mission->title) }}" required>
            </div>

            <div class="mb-3">
                <label for="description" class="form-label">Description <span class="text-danger">*</span></label>
                <textarea class="form-control" id="description" name="description" rows="4" required>{{ old('description', $mission->description) }}</textarea>
            </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select" id="location_id" name="location_id">
                        ...
                        @foreach($locations as $loc)
                            <option value="{{ $loc->id }}" {{ old('location_id', $mission->location_id) == $loc->id ? 'selected' : '' }}>{{ $loc->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control" id="location" name="location" value="{{ old('location', $mission->location) }}">
            ...
            <div class="row">
                <div class="col-md-4 mb-3">
                    ...
                    <input type="number" class="form-control" id="difficulty" name="difficulty" min="1" max="5" value="{{ old('difficulty', $mission->difficulty) }}" required>
                </div>
                <div class="col-md-4 mb-3">
                    <label for="image" class="form-label">Image Filename</label>
                    <input type="text" class="form-control" id="image" name="image" value="{{ old('image', $mission->image) }}">
                </div>
                <div class="col-md-4 mb-3">
                    ...
                    <select class="form-select" id="ship_id" name="ship_id">
                        ...
                        @foreach($ships as $ship)
                            <option value="{{ $ship->id }}" {{ old('ship_id', $mission->ship_id) == $ship->id ? 'selected' : '' }}>{{ $ship->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="mb-3 form-check">
                <input type="checkbox" class="form-check-input" id="featured" name="featured" value="1" {{ old('featured', $mission->featured) ? 'checked' : '' }}>
